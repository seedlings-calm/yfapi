...
func (r *RedisSets) GetImRedis() *redis.Client {
	return getRedis(IM_INSTANCE_NAME)
}

func (r *RedisSets) GetChatroomRedis() *redis.Client {
	return getRedis(CHATROOM_INSTANCE_NAME)
}

// GetUserRedis
//
//	@Description:	获取用户redis
//	@return			*redis.Client
func GetUserRedis() *redis.Client {
	return getRedis(USER_INSTANCE_NAME)
}

func GetImRedis() *redis.Client {
	return getRedis(IM_INSTANCE_NAME)
}

func GetChatroomRedis() *redis.Client {
	return getRedis(CHATROOM_INSTANCE_NAME)
}
...
func (r *RedisSets) ImLock(ctx context.Context, key string, lockTime time.Duration) (success bool, unlock func(), err error) {
	redisCli := r.getRedis(IM_INSTANCE_NAME)
	return baseLock(ctx, key, lockTime, redisCli)
}

func (r *RedisSets) ChatroomLock(ctx context.Context, key string, lockTime time.Duration) (success bool, unlock func(), err error) {
	redisCli := r.getRedis(CHATROOM_INSTANCE_NAME)
	return baseLock(ctx, key, lockTime, redisCli)
}
...
func ImLock(ctx context.Context, key string, lockTime time.Duration) (success bool, unlock func(), err error) {

	redisCli := getRedis(IM_INSTANCE_NAME)

	return baseLock(ctx, key, lockTime, redisCli)
}

func ChatroomLock(ctx context.Context, key string, lockTime time.Duration) (success bool, unlock func(), err error) {
	redisCli := getRedis(CHATROOM_INSTANCE_NAME)
	return baseLock(ctx, key, lockTime, redisCli)
}
