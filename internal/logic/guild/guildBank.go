...
// 添加银行卡
func (u *GuildBank) GuildBindBank(c *gin.Context, req *request_guild.BankBindReq) (code i18n_err.ErrCode) {
...
// guild提现
func (u *GuildBank) GuildWithdrawApply(c *gin.Context, req *user.UserWithdrawApplyReq) {
	userId := helper.GetUserId(c)
	...
	//查询会长补贴星光信息
	subsidyInfo, err := new(dao.UserAccountDao).GetUserAccountGuildSubsidy(userId, guildId)
	if err != nil {
		panic(i18n_err.I18nError{
	...
	//payAmount = payAmount.Mul(easy.StringToDecimal(cast.ToString(10)))
	// 扣除提现手续费
